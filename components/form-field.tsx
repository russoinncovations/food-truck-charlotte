...
export function FormField({
  ...
  return (
    <label className="block text-[15px] font-medium text-[#1E1E1E]">
      {label}
      {as === "textarea" ? (
        <textarea name={name} placeholder={placeholder} required={required} className={`${classes} min-h-28`} />
      ) : (
        <input name={name} type={type} placeholder={placeholder} required={required} className={classes} />
      )}
